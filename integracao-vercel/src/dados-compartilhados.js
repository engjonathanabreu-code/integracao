...
export function projetar(base, local) {
  ...
  merge('usuarios', base.profiles.map(p => bind('usuarios',p,{id:userId(p.id),erpRef:p.id,nome:p.nome,email:p.email || '',tipoERP:p.tipo,setor:sectors[p.tipo] || 'consulta',funcao:p.tipo?.startsWith('Diretor') || p.tipo === 'Administrador' ? 'Diretor' : 'Analista',ativo:p.ativo !== false,origem:'ERP',online:false,agendaPessoal:[],calendarioOculto:[]},{nome:'nome',email:'email',ativo:'ativo'},'profiles')));
  ...
  merge('metas',base.metas.map(r=>{
    ...
    return bind('metas',r,{id:r.id,erpId:r.id,titulo:r.titulo,observacoes:text(r.observacoes),semana_inicio:text(r.semana_inicio),prazo:text(r.prazo),status:r.status,setor:base.meta_setores.find(s=>s.id===r.setor_id)?.nome||'',associacao_tipo:a.tipo,associacao_id:a.id||'',nucleos:a.tipo==='nucleo'?[a.id]:[],criadoPor:uid(r.created_by),icone:text(r.icone),responsaveis:group(base.meta_responsaveis,'meta_id',r.id).map(x=>uid(x.usuario_id)),
      ...
      comentarios:group(base.meta_comentarios,'meta_id',r.id).map(comment),historico:group(base.meta_historico,'meta_id',r.id).map(x=>({id:x.id,acao:x.acao,descricao:text(x.descricao),autor:names[x.autor_id]||'Equipe',data:x.created_at})),
