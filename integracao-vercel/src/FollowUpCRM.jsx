...
export default function FollowUpCRM({card,usuarios,atualizar}){
 const m=useModulo(()=>listarCRM('integracao_crm_followups',`&card_id=eq.${card.id}&order=criado_em.desc,id.desc`),[card.id],['crm']);
 ...
 <p className={`crm-followup-status ${situacaoFollowup(pendente).tipo}`}>{situacaoFollowup(pendente).texto}{pendente&&<> · {dataFollowup(pendente.previsto_em)}</>}</p>
 <form className="crm-form" data-edicao-pendente={!!dias||!!resumo} onSubmit={async e=>{e.preventDefault();if(pendente&&!feito)return;const ok=await m.executar(()=>rpcCRM('integracao_crm_followup',{p_card:card.id,p_pedido:pedido,p_anterior:pendente?.id||null,p_dias:Number(dias),p_resumo:pendente?resumo.trim():''}));if(ok){setDias('');setResumo('');setFeito(false);setPedido(crypto.randomUUID());setAviso(pendente?'FollowUp concluído e próximo prazo agendado.':'FollowUp agendado.');atualizar();}}}>
