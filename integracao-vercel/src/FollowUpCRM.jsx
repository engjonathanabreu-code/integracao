...
export default function FollowUpCRM({card,usuarios,atualizar,tabela='integracao_crm_followups',rpc='integracao_crm_followup',encerrado=false}){
 ...
 <p className={`crm-followup-status ${situacaoFollowup(pendente).tipo}`}>{encerrado?'Negócio encerrado':situacaoFollowup(pendente).texto}{pendente&&<> · {dataFollowup(pendente.previsto_em)}</>}</p>
 ...
 </fieldset></form>}{encerrado&&<p>Negócio encerrado. O histórico permanece disponível.</p>}{aviso&&<p role="status">{aviso}</p>}
 <details><summary>Histórico de FollowUp ({m.dados.filter(f=>f.status!=='pendente').length})</summary>{m.dados.filter(f=>f.status!=='pendente').map(f=><article className="crm-mensagem" key={f.id}><strong>{f.status==='feito'?'Feito':f.status==='cancelado'?'Agendamento encerrado junto com o negócio':'Agendamento unificado ao vincular cliente'}</strong><p>Previsto: {dataFollowup(f.previsto_em)} · {f.prazo_dias} dia(s)</p>{f.status==='feito'&&<><p>{dataFollowup(f.concluido_em)} · {autor(f.concluido_por)} · {new Date(f.concluido_em)>new Date(f.previsto_em)?'Concluído com atraso':'Concluído no prazo'}</p><p>{f.resumo}</p></>}</article>)}</details></>}</section>;
}
