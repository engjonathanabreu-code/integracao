...
export default function NegociacaoCRM({card,ocupado,onSalvar}){
 ...
 const salvar=async e=>{e.preventDefault();try{const dados={status,...prepararNegociacao(form)};if(await onSalvar(dados,base)){setBase({...base,...dados});setForm(formularioNegociacao(dados));}}catch(e){setErro(e.message);}};
 return <form className="crm-negociacao" aria-label={`Negociação de ${card.nome||card.lead_nome||'Contato'}`} onSubmit={salvar}>
  ...
  <button className="btn btn-primario" disabled={ocupado||!mudou}>Salvar negociação e status</button>
