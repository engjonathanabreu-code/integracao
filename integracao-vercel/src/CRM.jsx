...
export default function CRM({usuario,db,ir,abrirCliente}) {
  ...
  const card=c=><article className="crm-card" key={c.id}><h3>{nomeCard(c)}</h3><p>{c.cpf_cnpj||'Documento não informado'}</p><p>{c.telefone||c.lead_telefone||'Telefone não informado'}</p><p>{c.municipio||c.lead_cidade||'Município a confirmar'}{c.remessa?` · ${c.remessa}`:''}</p>
    <label>Status<select className="inp" aria-label={`Status de ${nomeCard(c)}`} value={c.status} disabled={m.ocupado} onChange={e=>m.executar(()=>editarCRM('integracao_crm_cards',c.id,{status:e.target.value}))}>{ETAPAS_CRM.map(s=><option key={s}>{s}</option>)}</select></label>
  ...
  return <div className="contem largo"><div className="cabeca"><div><h1>CRM</h1><p>Relacionamento, atendimentos e próximos passos.</p></div><button className="btn btn-primario" onClick={()=>ir({pag:'municipios'})}>Cadastrar cliente no município</button></div>
    <div className="crm-nav">{[['funil','Funil comercial'],['leads','Potenciais leads'],['tarefas','Tarefas abertas']].map(([id,nome])=><button key={id} className={`btn ${aba===id?'btn-primario':''}`} onClick={()=>{setAba(id);setHistorico(null);setForm(null);}}>{nome}</button>)}</div>
    ...
    {m.dados&&aba==='leads'&&<div className="crm-grade">{cards.filter(c=>!c.cliente_id).map(card)}{!cards.some(c=>!c.cliente_id)&&<p>Nenhum potencial lead.</p>}</div>}
