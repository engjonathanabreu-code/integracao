...
import {X,Phone,FileText,MapPin,Layers,MessageSquare,ListTodo,Filter,UserPlus,UserX,BarChart3,Building2,Trash2,Archive,Pencil} from 'lucide-react';
import {listarCRM,criarCRM,editarCRM,rpcCRM} from './crm-api.js';
import {ETAPAS_FUNIL,acessoCRM,normalizarCRM} from './crm-regras.js';
import './crm.css';
...
export default function CRM({usuario,db,ir,abrirCliente}) {
  ...
  const statusCard=c=><NegociacaoCRM key={c.id} card={c} ocupado={m.ocupado} onSalvar={async(dados,anterior)=>ativado(c,dados.status,await m.executar(()=>salvarNegociacaoCRM(c.id,dados,anterior)))} onConverter={async(fn,status)=>ativado(c,status,await m.executar(fn))}/>;
  ...
  return <div className="contem largo"><div className="cabeca"><div><h1>CRM</h1><p>Relacionamento, atendimentos e próximos passos.</p></div>{aba!=='institucionais'&&<button className="btn btn-primario" onClick={()=>setNovoLead(true)}><UserPlus size={17}/>Cadastrar lead</button>}</div>
    <div className="crm-nav">{[['funil','Funil comercial',Filter],['leads','Potenciais leads',UserPlus],['institucionais','Clientes institucionais',Building2],['tarefas','Tarefas abertas',ListTodo],['perdidos','Perdidos',UserX],...(acesso.admin?[['dashboard','Dashboard comercial',BarChart3],['arquivo','Arquivo do CRM',Archive]]:[])].map(([id,nome,Icone])=><button key={id} className={`btn ${aba===id?'btn-primario':''}`} onClick={()=>{setAba(id);setHistorico(null);setForm(null);}}><Icone size={17} aria-hidden="true"/>{nome}</button>)}</div>
    {!['dashboard','institucionais','arquivo'].includes(aba)&&<div className="crm-acoes"><input className="inp" aria-label="Buscar no CRM" placeholder="Nome, CPF, telefone ou município" value={busca} onChange={e=>setBusca(e.target.value)}/>{acesso.admin&&<select className="inp" aria-label="Filtrar comercial" value={responsavel} onChange={e=>setResponsavel(e.target.value)}><option value="">Todos os comerciais</option>{comerciais.map(u=><option key={u.id} value={u.erpRef}>{u.nome}</option>)}</select>}</div>}
    ...
    {aba==='funil'&&<p className="ajuda">Clientes marcados como Cliente ativo saem do funil e ficam registrados{acesso.admin?' no relatório Clientes ativados do Dashboard comercial':' para os relatórios de desempenho'}.</p>}
    ...
    {edicao&&<FichaCliente titulo={`Editar ${nomeCard(edicao)}`} compacta ocupado={m.ocupado} fechar={()=>setEdicao(null)}><EditarCardCRM card={edicao} nucleos={db.nucleos||[]} ocupado={m.ocupado} erro={erroEdicao} cancelar={()=>setEdicao(null)} salvar={async dados=>{setErroEdicao('');if(await m.executar(async()=>{try{await rpcCRM('integracao_crm_editar_card',dados);if(edicao.cliente_id)invalidarIndiceClientes();}catch(e){setErroEdicao(e.message);throw e;}})){setEdicao(null);setAvisoLead(`Dados de ${dados.p_nome} atualizados.`);}}}/></FichaCliente>}
    ...
    {aba==='arquivo'&&acesso.admin&&<ArquivoCRM usuarios={db.usuarios} atualizar={m.atualizar} historico={id=><HistoricoAtendimento cardId={id} usuario={usuario}/>}/>}
