...
async function marcas(db,dia,horas){for(let i=0;i<horas.length;i++)await db.query(`insert into integracao_ponto_batidas values(gen_random_uuid(),$1,$2,$3)`,[clt,`${dia}T${horas[i]}:00-03:00`,i%2?'saida':'entrada']);}
test('jornada, batida do servidor, idempotência, alternância e restrição de acesso',async()=>{const db=await banco();try{
 ...
 await assert.rejects(()=>como(db,clt,`insert into integracao_ponto_batidas values(gen_random_uuid(),'${clt}',now(),'saida')`));
