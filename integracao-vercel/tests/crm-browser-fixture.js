...
export function instalarCRMFixture(base) {
 ...
 return (url,options,json)=>{
  ...
  if(table==='integracao_crm_cadastrar_lead'){const p=JSON.parse(options.body);base.integracao_crm_cards.push({id:p.p_id,lead_nome:p.p_nome,lead_telefone:p.p_telefone,lead_municipio_id:p.p_municipio,municipio_id:p.p_municipio,municipio:base.fin_receb_municipios.find(m=>m.id===p.p_municipio).nome,status:'Cliente novo',responsavel_id:p.p_responsavel||id(1),origem:'manual'});return json(p.p_id);}
