...
test('endpoints verificam sessão e perfil antes da IA e não aceitam respostas incompletas',async()=>{
 const original=global.fetch,key=process.env.ANTHROPIC_API_KEY;process.env.ANTHROPIC_API_KEY='test-only';let perfil={tipo:'Projetos',setor:'Projetos',ativo:true},calls=0,stop='end_turn';
 global.fetch=async url=>{if(url.includes('/auth/'))return Response.json({id:'teste'});if(url.includes('/profiles?'))return Response.json([perfil]);if(url.includes('/storage/'))return new Response('Ofício solicitando documentos à prefeitura de teste.');calls++;return Response.json({stop_reason:stop,content:[{type:'text',text:url.includes('anthropic')?'Conteúdo do ofício para revisão.':''}]});};
 ...
 assert.equal(calls,1);perfil={tipo:'Jurídico',ativo:true};stop='max_tokens';assert.equal((await invoke(gerar,body)).code,503);
 assert.equal((await invoke(gerar,{...body,orientacao:'x'.repeat(10001)})).code,400);
 }finally{global.fetch=original;if(key===undefined)delete process.env.ANTHROPIC_API_KEY;else process.env.ANTHROPIC_API_KEY=key;}
});
