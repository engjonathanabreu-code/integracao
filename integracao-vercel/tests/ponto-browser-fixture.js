...
export function instalarPontoFixture(){
 ...
 let solicitacoes=[];window.pedidosPonto=solicitacoes;
 if(new URLSearchParams(location.search).has('diretorPedido'))solicitacoes.push({id:id(955),usuario_id:id(2),dia:ontem,batidas:[ontem+'T08:00:00-03:00',ontem+'T17:00:00-03:00'],anteriores:[],motivo:'Esqueci de marcar no campo',status:'pendente',criado_em:new Date().toISOString()});
 ...
 return (url,options,json)=>{const table=url.pathname.split('/').at(-1);if(table==='integracao_ponto'){if(!navigator.onLine)throw new TypeError('Failed to fetch');
 const {p_acao:a,p_dados:d}=JSON.parse(options.body);
 if(a==='estado')return json({jornada,proximo:batidas.length%2?'saida':'entrada',hoje:{dia:hoje,batidas:batidas.map(b=>b.ocorrido_em),incompleto:batidas.length%2===1}});
 if(a==='bater'||a==='bater_offline'){const existente=batidas.find(b=>b.id===d.pedido);if(existente)return json(existente);const r={id:d.pedido,tipo:d.tipo,ocorrido_em:d.ocorrido_em||new Date().toISOString()};batidas.push(r);return json(r);}
 if(a==='solicitar_correcao'){const r={id:d.pedido,usuario_id:id(1),dia:d.dia,batidas:d.batidas,anteriores:dia().batidas,motivo:d.motivo,status:'pendente',criado_em:new Date().toISOString()};solicitacoes.push(r);return json(r);}
 if(a==='cancelar_correcao'||a==='decidir_correcao'){const r=solicitacoes.find(x=>x.id===d.pedido);Object.assign(r,{status:a==='cancelar_correcao'?'cancelada':d.aprovada?'aprovada':'recusada',motivo_decisao:d.motivo||'Cancelada pelo solicitante',decisor:id(1),decidido_em:new Date().toISOString()});return json(r);}
