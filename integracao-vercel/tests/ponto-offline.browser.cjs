...
await p.goto('http://127.0.0.1:5178/tests/browser.html?ponto');await login();await p.getByRole('button',{name:/^Bater ponto Registrar/}).click();await p.getByRole('button',{name:'Bater ponto — entrada',exact:true}).waitFor();
...
await p.reload();await login();await p.getByRole('button',{name:/^Bater ponto Registrar/}).click();await p.getByRole('button',{name:'Bater ponto — saida',exact:true}).waitFor();await p.getByText(/1 marcação\(ões\) salva/).waitFor();
...
await p.getByRole('button',{name:'Fechar ponto',exact:true}).click();await p.getByRole('button',{name:'Clientes',exact:true}).click();
