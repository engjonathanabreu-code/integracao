...
import { lerGeoJSON, prepararFeicoes, salvarImportacao, vincularFeicao } from './levantamento.js';
...
export default function ImportadorGeoJSON({db,n,perm,mutar,por,setToast,onVer}) {
  ...
  const executar=fn=>{try{setErro('');fn();}catch(e){setErro(e.message);}};
  ...
  const salvar=()=>executar(()=>{
    ...
    mutar(d=>salvarImportacao(d,n.id,previa,por),'Levantamento GeoJSON importado',{nucleoId:n.id,municipioId:n.municipioId,detalhe:`${previa.arquivo}: ${previa.feicoes.length} feições, campo ${previa.campo}`});
    setPrevia(null);setLido(null);setToast('Levantamento salvo. Agora vincule os lotes.');
  });
  ...
  const vincular=()=>executar(()=>{
    if(!pode)throw new Error('Vinculação disponível na etapa Topografia.');
    mutar(d=>vincularFeicao(d,n.id,confirma.feicao,confirma.alvo,confirma.anterior,por),'Feição vinculada ao memorial',{nucleoId:n.id,municipioId:n.municipioId,processoId:confirma.alvo.moradorId,detalhe:`${confirma.feicao.codigo} → ${confirma.alvo.nome}`});
    setConfirma(null);setDestino('');setToast('Vínculo salvo. Vértices e medidas disponíveis nos Memoriais e no PRF.');
  });
  ...
  return <section className="card" style={{padding:18,minWidth:0}}>
    <div className="flex flex-wrap items-center gap-2"><h3 style={{margin:0,flex:1}}>Lotes e áreas do GeoJSON</h3><button className="btn" disabled={!pode||!!salvo?.feicoes?.length} onClick={()=>entrada.current?.click()}><Upload size={15}/>Importar GeoJSON</button></div>
    ...
    {!pode&&<p className="ajuda">Importação e vínculo disponíveis para Topografia e Diretoria durante a etapa Topografia. O histórico permanece acessível depois.</p>}
    ...
    {feicao&&<div style={{marginTop:16}}><h4>{feicao.codigo} — vértices e lados</h4><div className="rolagem"><table className="tab"><thead><tr>{['Estação','Vante','Este (E)','Norte (N)','Longitude','Latitude','Azimute','Distância (m)'].map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{feicao.vertices.map((v,i)=><tr key={i}><td>{v.nome}</td><td>{feicao.vertices[(i+1)%feicao.vertices.length].nome}</td><td>{formatarCoordenada(v.e)}</td><td>{formatarCoordenada(v.n)}</td><td>{v.longitude}</td><td>{v.latitude}</td><td>{v.azimute}</td><td>{formatarMedida(v.distancia)}</td></tr>)}</tbody></table></div>
      ...
      {confirma&&<div role="region" aria-label="Confirmar vínculo" style={{marginTop:12,padding:12,border:'1px solid var(--border)',borderRadius:8}}><p><strong>{confirma.feicao.codigo} → {confirma.alvo.nome}</strong></p><p>Serão preenchidos os vértices, área, perímetro e texto do memorial deste destino. {confirma.anterior?.memorial||confirma.anterior?.texto?'O memorial existente será substituído.':'Os demais dados do cadastro serão preservados.'}</p><div className="flex gap-2"><button className="btn btn-primario" onClick={vincular}>Confirmar vínculo e preencher memorial</button><button className="btn" onClick={()=>setConfirma(null)}>Cancelar</button></div></div>}
    </div>}
