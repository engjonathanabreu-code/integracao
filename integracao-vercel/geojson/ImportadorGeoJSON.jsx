import RevogarMemorial from '../memoriais/RevogarMemorial.jsx';
import {memorialDoDestino} from '../memoriais/revogacao.js';
import { useRef, useState } from 'react';
import { Upload, MapPin } from 'lucide-react';
import { lerGeoJSON, prepararFeicoes, salvarImportacao, vincularFeicao, sugerirVinculos, confirmarSugestoes, memorialDaFeicao } from './levantamento.js';
...
export default function ImportadorGeoJSON({db,n,perm,mutar,por,setToast,onVer,onMemoriais}) {
  ...
  const [sugestoes,setSugestoes]=useState(null),[marcadas,setMarcadas]=useState([]);
  ...
  const ler=async arquivo=>{
    ...
    try{
      ...
      setLido({...novo,arquivo:arquivo.name});setCampo(novo.campos.find(k=>k.toLowerCase()==='q_l')||novo.campos.find(k=>k.toLowerCase()==='lote')||'');
  ...
  const confirmarAutomaticos=()=>executar(async()=>{if(!pode)throw new Error('Sem permissão para vincular.');const escolhidas=sugestoes.filter(s=>marcadas.includes(s.feicao.id));if(!escolhidas.length)return;await mutar(d=>confirmarSugestoes(d,n.id,escolhidas,por),'Vínculos de memoriais confirmados',{nucleoId:n.id,municipioId:n.municipioId,detalhe:escolhidas.map(s=>s.feicao.codigo+' → '+s.unidade.codigo).join('; ')});setSugestoes(null);setMarcadas([]);setToast('Vínculos confirmados e memoriais preenchidos.');});
  ...
  return <section className="card" style={{padding:18,minWidth:0}}>
    ...
    {!previa&&pode&&salvo&&<button className="btn" style={{marginTop:12}} onClick={()=>{setSugestoes(sugerirVinculos(feicoes,unidades));setMarcadas([]);}}>Sugerir vínculos pelos códigos</button>}
    {sugestoes&&<section className="card" style={{padding:16,marginTop:12}} aria-label="Revisar sugestões de vínculo"><h4>Confirme os moradores antes de vincular</h4><p className="ajuda">Códigos sem correspondência, repetidos ou ambíguos exigem seleção manual. Nenhum vínculo foi salvo.</p><div className="rolagem"><table className="tab"><thead><tr><th>Confirmar</th><th>Lote</th><th>Código do arquivo</th><th>Morador / unidade sugerido</th></tr></thead><tbody>{sugestoes.map(s=><tr key={s.feicao.id}><td><input type="checkbox" aria-label={`Confirmar ${s.feicao.codigo}`} disabled={!s.unidade} checked={marcadas.includes(s.feicao.id)} onChange={e=>setMarcadas(xs=>e.target.checked?[...xs,s.feicao.id]:xs.filter(x=>x!==s.feicao.id))}/></td><td>{s.feicao.codigo}</td><td>{s.feicao.codigoMorador||'Não informado'}</td><td>{s.unidade?`${s.unidade.codigo} — ${s.unidade.requerente}${s.unidade.memorial?' · substituirá o memorial existente':''}`:s.motivo}</td></tr>)}</tbody></table></div><div className="flex flex-wrap gap-2" style={{marginTop:12}}><button className="btn btn-primario" disabled={!marcadas.length} onClick={confirmarAutomaticos}>Confirmar e salvar {marcadas.length} vínculos selecionados</button><button className="btn" onClick={()=>{setSugestoes(null);setMarcadas([]);}}>Cancelar sugestões</button></div></section>}
    ...
    {feicao&&<div style={{marginTop:16}}><h4>{feicao.codigo} — vértices e lados</h4>
      ...
      {feicao.vinculo&&<><p className="ajuda">Para editar os vértices e confrontantes, abra este destino na aba Memoriais. Para corrigir o destino, revogue o vínculo e escolha o novo morador ou área.</p><RevogarMemorial db={db} n={n} alvo={{...feicao.vinculo,viaId:feicao.id}} anterior={memorialDoDestino(db,n.id,{...feicao.vinculo,viaId:feicao.id})} pode={pode} mutar={mutar} por={por} setToast={setToast} aoRevogar={()=>{setConfirma(null);setSugestoes(null);}}/></>}
      ...
      <details style={{marginTop:16}}><summary>Ver vértices e medidas</summary><div className="rolagem"><table className="tab"><thead><tr>{['Estação','Vante','Este (E)','Norte (N)','Longitude','Latitude','Azimute','Distância (m)'].map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{feicao.vertices.map((v,i)=><tr key={i}><td>{v.nome}</td><td>{feicao.vertices[(i+1)%feicao.vertices.length].nome}</td><td>{formatarCoordenada(v.e)}</td><td>{formatarCoordenada(v.n)}</td><td>{v.longitude}</td><td>{v.latitude}</td><td>{v.azimute}</td><td>{formatarMedida(v.distancia)}</td></tr>)}</tbody></table></div></details>
    </div>}
