...
BEGIN
 ...
 IF etapa<>'2' THEN result=result||jsonb_build_object('requerente',jsonb_build_object('tipoPessoa',d#>'{requerente,tipoPessoa}')); END IF;
...
CREATE OR REPLACE FUNCTION public.integracao_moradores_carga(municipio uuid DEFAULT NULL, resumo boolean DEFAULT true, inicio integer DEFAULT 0)
...
BEGIN
 ...
 INTO clientes FROM public.fin_receb_clientes c WHERE municipio IS NULL OR c.municipio_id=municipio;
 ...
 WHERE e.colecao='processos' AND (municipio IS NULL OR e.referencia_id IN (SELECT c.id FROM public.fin_receb_clientes c WHERE c.municipio_id=municipio) OR e.dados->>'municipioId'=municipio::text) ORDER BY e.registro_id LIMIT 500 OFFSET greatest(inicio,0))e;
 IF inicio=0 THEN SELECT count(*) INTO total FROM public.integracao_moradores e WHERE e.colecao='processos' AND (municipio IS NULL OR e.referencia_id IN (SELECT c.id FROM public.fin_receb_clientes c WHERE c.municipio_id=municipio) OR e.dados->>'municipioId'=municipio::text); END IF;
 RETURN jsonb_build_object('clientes',clientes,'complementos',complementos,'total',total);
END $fn$;
REVOKE ALL ON FUNCTION public.integracao_moradores_carga(uuid,boolean,integer) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.integracao_moradores_carga(uuid,boolean,integer) TO authenticated;
