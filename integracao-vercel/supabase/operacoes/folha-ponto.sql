begin;
create table public.integracao_ponto_jornadas (
 id uuid primary key default gen_random_uuid(), usuario_id uuid not null references public.profiles(id),
 vigencia date not null, vinculo text not null check(vinculo in ('CLT','Contrato')),
 dias integer[] not null, entrada time not null, saida time not null, intervalo integer not null,
 autor uuid not null references public.profiles(id), criado_em timestamptz not null default clock_timestamp(),
 check(dias <@ array[0,1,2,3,4,5,6] and cardinality(dias)>0),
 check(saida>entrada and intervalo>=0 and intervalo<extract(epoch from(saida-entrada))/60)
);
create index on public.integracao_ponto_jornadas(usuario_id,vigencia desc,criado_em desc);
create table public.integracao_ponto_batidas (
 id uuid primary key, usuario_id uuid not null references public.profiles(id),
 ocorrido_em timestamptz not null, tipo text not null check(tipo in ('entrada','saida'))
);
create index on public.integracao_ponto_batidas(usuario_id,ocorrido_em);
create table public.integracao_ponto_revisoes (
 id uuid primary key default gen_random_uuid(),usuario_id uuid not null references public.profiles(id),dia date not null,
 batidas timestamptz[] not null,motivo text not null check(length(trim(motivo))>=5),autor uuid not null references public.profiles(id),criado_em timestamptz not null default clock_timestamp()
);
create index on public.integracao_ponto_revisoes(usuario_id,dia,criado_em desc);
create table public.integracao_ponto_decisoes (
 id uuid primary key default gen_random_uuid(),usuario_id uuid not null references public.profiles(id),dia date not null,
 assinatura text not null,minutos integer not null check(minutos>0),aprovada boolean not null,
 motivo text not null check(length(trim(motivo))>=5),autor uuid not null references public.profiles(id),criado_em timestamptz not null default clock_timestamp()
);
create index on public.integracao_ponto_decisoes(usuario_id,dia,criado_em desc);
create table public.integracao_ponto_ajustes (
 id uuid primary key default gen_random_uuid(),usuario_id uuid not null references public.profiles(id),dia date not null,
 minutos integer not null check(minutos between -1440 and 1440 and minutos<>0),
 motivo text not null check(length(trim(motivo))>=5),autor uuid not null references public.profiles(id),criado_em timestamptz not null default clock_timestamp()
);
create index on public.integracao_ponto_ajustes(usuario_id,dia);
do $$ declare t text; begin
 foreach t in array array['integracao_ponto_jornadas','integracao_ponto_batidas','integracao_ponto_revisoes','integracao_ponto_decisoes','integracao_ponto_ajustes'] loop
 execute format('alter table public.%I enable row level security',t);
 execute format('revoke all on public.%I from public,anon,authenticated',t);
 execute format('grant select on public.%I to authenticated',t);
 execute format('create policy ponto_leitura on public.%I for select to authenticated using (integracao_crm_privado.permite(''admin'') or (usuario_id=auth.uid() and exists(select 1 from public.profiles where id=auth.uid() and ativo)))',t);
 end loop; end $$;

create function integracao_crm_privado.ponto_dia(p_usuario uuid,p_dia date) returns jsonb
...
begin
 ...
 select coalesce(array_agg(ocorrido_em order by ocorrido_em),'{}'),coalesce(jsonb_agg(jsonb_build_object('tipo',tipo,'hora',ocorrido_em) order by ocorrido_em),'[]') into b,originais
...
create function integracao_crm_privado.ponto_operar(p_acao text,p_dados jsonb) returns jsonb
...
begin
 ...
 select * into ultima from public.integracao_ponto_batidas where usuario_id=u order by ocorrido_em desc limit 1;
 ...
 insert into public.integracao_ponto_batidas values(novo_id,u,instante,p_dados->>'tipo') returning * into recibo;
...
create function public.integracao_ponto(p_acao text,p_dados jsonb default '{}') returns jsonb language sql security invoker set search_path='' as $$select integracao_crm_privado.ponto_operar(p_acao,p_dados)$$;
revoke all on function integracao_crm_privado.ponto_dia(uuid,date),integracao_crm_privado.ponto_operar(text,jsonb),public.integracao_ponto(text,jsonb) from public,anon;
grant execute on function integracao_crm_privado.ponto_dia(uuid,date),integracao_crm_privado.ponto_operar(text,jsonb),public.integracao_ponto(text,jsonb) to authenticated;
